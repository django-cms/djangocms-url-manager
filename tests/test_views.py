...
class UrlManagerSelect2ContentObjectViewsTestCase(BaseUrlTestCase):
    ...
    @skipIf(CMS_36, "Test relevant only for CMS>=4.0")
    @skipUnless(
        BaseUrlTestCase.is_versioning_enabled(),
    ...
    def test_raise_error_when_return_unregistered_user_model_in_select2_view(
        self
    ):
        with self.login_user_context(self.superuser):
            with self.assertRaises(ValueError):
                self.client.get(
                    self.select2_endpoint,
                    data={
                        "content_id": ContentType.objects.get_for_model(
                            User
                        ).id
                    },
                )
    ...
    def test_select2_view_text_poll_content_repr(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json()["results"][0]["text"], str(self.poll_content)
        )
    ...
    def test_select2_view_site_for_cms40(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            [a["id"] for a in response.json()["results"]], [self.page2.pk]
        )
...
class UrlManagerSelect2UrlsViewsTestCase(BaseUrlTestCase):
    ...
    def test_select2_url_view_with_site_id(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            [p["id"] for p in response.json()["results"]], [self.url2.pk]
        )
    ...
    def test_select2_url_view_with_object_pk(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            [a["id"] for a in response.json()["results"]], [self.url2.pk]
        )
    ...
    def test_select2_url_view_set_limit(self):
        ...
        with self.login_user_context(self.superuser):
            response = self.client.get(
                self.select2_urls_endpoint, data={"limit": 2}
            )
        content = response.json()
        self.assertEqual(response.status_code, 200)
        self.assertTrue(content["more"])
